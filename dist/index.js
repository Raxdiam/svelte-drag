...
var draggable = (node, options = {}) => {
  ...
  function dragStart(e) {
    ...
    if (cancelEl == null ? void 0 : cancelEl.contains(dragEl))
      throw new Error("Element being dragged can't be a child of the element on which action is applied");
...
function fireSvelteDragStopEvent(node) {
  node.dispatchEvent(new CustomEvent("svelte-drag:end"));
}
function fireSvelteDragStartEvent(node) {
  node.dispatchEvent(new CustomEvent("svelte-drag:start"));
}
function fireSvelteDragEvent(node, translateX, translateY) {
  node.dispatchEvent(new CustomEvent("svelte-drag", { detail: { offsetX: translateX, offsetY: translateY } }));
}
function setupListeners(dragStart, dragEnd, drag) {
...
var computeBoundRect = memoize_default((bounds, rootNode) => {
